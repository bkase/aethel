...
pub async fn insert_artifact(pool: &Pool<Sqlite>, uuid: &Uuid, filepath: &str) -> Result<()> {
    sqlx::query("INSERT INTO artifacts (uuid, filepath) VALUES (?, ?)")
        .bind(uuid.to_string())
        .bind(filepath)
        .execute(pool)
        .await?;
...
pub async fn get_artifact_path(pool: &Pool<Sqlite>, uuid: &Uuid) -> Result<Option<String>> {
    let result = sqlx::query_as::<_, (String,)>("SELECT filepath FROM artifacts WHERE uuid = ?")
        .bind(uuid.to_string())
        .fetch_optional(pool)
        .await?;
...
pub async fn delete_artifact(pool: &Pool<Sqlite>, uuid: &Uuid) -> Result<()> {
    sqlx::query("DELETE FROM artifacts WHERE uuid = ?")
        .bind(uuid.to_string())
        .execute(pool)
        .await?;
...
pub async fn rebuild_index(pool: &Pool<Sqlite>, artifacts: Vec<(Uuid, String)>) -> Result<()> {
    sqlx::query("DELETE FROM artifacts").execute(pool).await?;
