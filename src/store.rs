...
pub fn parse_artifact(content: &str) -> Result<Artifact> {
    ...
    if parts.len() < 3 {
        return Err(AethelError::ValidationError("Invalid artifact format: missing frontmatter".to_string()));
    }
...
pub fn get_plugin_artifact_dir(vault_path: &Path, plugin_id: &str) -> PathBuf {
    vault_path.join("20_artifacts").join(plugin_id)
}
